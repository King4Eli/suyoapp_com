...
async function verifyActiveMatchWith(req, targetUserID, matchId) {
  ...
  const matchRows = await db
    .select({ match_status: matches.matchStatus })
    .from(matches)
    .where(
      and(
        ...
        or(
          and(
            eq(matches.matchUserIdFrom, callerID),
            eq(matches.matchUserIdTo, targetUserID),
          ),
          and(
            eq(matches.matchUserIdFrom, targetUserID),
            eq(matches.matchUserIdTo, callerID),
          ),
        ),
      ),
    );
  ...
  const matchRows = await db
    ...
    .where(
      and(
        eq(matches.matchId, matchId),
        or(
          eq(matches.matchUserIdFrom, userID),
          eq(matches.matchUserIdTo, userID),
        ),
      ),
    );
