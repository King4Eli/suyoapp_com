...
export default async function pushFeedPost(data) {
  ...
  try {
    ...
    // post_media is a native JSON column -- Drizzle's json() stringifies on
    // write, so pass the array itself, not a pre-stringified string.
    const [result] = await db.insert(feedPosts).values({
      postId,
      postUserId: sessions.currentUserID,
      postCaption: caption || null,
      postMedia: media.length > 0 ? media : null,
    });
