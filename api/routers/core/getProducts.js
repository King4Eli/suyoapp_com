...
const PRODUCTS_CACHE_TTL_SECONDS = envInt(
...
export default async function getProducts() {
  ...
  try {
    const cached = await redisDo(async (client) =>
      client.get(namer.redis.products),
    ).catch((err) => {
      tools.serverLog(
        `Redis read failed in getProducts: ${err}`,
        "getProducts-1",
      );
      return null;
    });
    ...
    // list products
    const [productRows] = await db.execute(sql`SELECT
                JSON_OBJECT(
    ...
    await redisDo(async (client) => {
      await client.set(namer.redis.products, JSON.stringify(productRows), {
        EX: PRODUCTS_CACHE_TTL_SECONDS,
      });
    }).catch((err) => {
      tools.serverLog(
        `Redis write failed in getProducts: ${err}`,
        "getProducts-2",
      );
    });
  } catch (err) {
