  ...
  static maskEmail(email = "") {
    ...
    const maskedLocal =
      local.length <= 2
        ...
        : local.slice(0, 2) +
          "*".repeat(Math.max(local.length - 3, 1)) +
          local.slice(-1);
  ...
  static serverLog(message = "", title = "log_") {
    ...
    (async () => {
      try {
        ...
        const [jj] = await db.insert(logsApplication).values({
          reportId: id,
        ...
        if (!jj || jj?.affectedRows === 0) {
          console.error(
            ">> 🔴 🔴 Failed to log message to database: No rows affected",
          );
        } else {
          console.log(
            "\n--",
            `[${new Date().toISOString()}]`,
            ">> logged server Error ID:",
            id,
          );
        }
      } catch (e) {
