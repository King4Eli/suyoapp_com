...
export function Screen_editProfilePrompts({
  navigation,
  route,
}: {
  navigation: any;
  ...
  const onSave: ((updated: PromptEntry[]) => void) | undefined =
    route.params?.onSave;

  const [prompts, setPrompts] = useState<PromptEntry[]>(existingPrompts);
  const [availablePrompts, setAvailablePrompts] = useState<
    Array<{ id_ai: number; question: string }>
  >([]);
  ...
  const savePrompt = (
    question: { id_ai: number; question: string },
    answer: string,
  ) => {
            ...
            <View style={styles.section}>
              ...
              {!isLoading && hasMore && (
                <Pressable
                  ...
                  onPress={() =>
                    loadMore([
                      ...prompts.map(p => p.id_ai),
                      ...availablePrompts.map(p => p.id_ai),
                    ])
                  }
                >
