...
type InterestGroup = {
  category: string;
  items: Array<{ id_ai: number; interested_in: string }>;
};

export function Screen_editProfileInterests({
  navigation,
  route,
}: {
  navigation: any;
  ...
  const existingInterests: InterestEntry[] =
    route.params?.existingInterests ?? [];
  const onSave: ((updated: InterestEntry[]) => void) | undefined =
    route.params?.onSave;

  const [interests, setInterests] =
    useState<InterestEntry[]>(existingInterests);
    ...
    (async () => {
      try {
        const response = await _http_request({
          customApiUrl:
            __CONFIG__.HTTPS_API_DOMAIN + '/api/core/v1/getInterests',
          reqType: 'POST',
        });
  ...
  const toggleInterest = (item: { id_ai: number; interested_in: string }) => {
    setInterests(prev => {
      if (prev.some(v => v.id_ai === item.id_ai))
        return prev.filter(v => v.id_ai !== item.id_ai);
      if (prev.length >= MAX_INTERESTS) {
        Toastx.show({
          message: `Max ${MAX_INTERESTS} interests allowed.`,
          type: 'info',
        });
        return prev;
      }
