...
export class SocketClient {
  ...
  static connect(userID: string, callback?: (data: any) => void) {
    console.log(
      `🟨 [SOCKET] connect() called -> userID=${userID} host=${
        __CONFIG__.HTTPS_API_DOMAIN
      } alreadyConnected=${Boolean(this.socket?.connected)}`,
    );
    ...
    this.socket = io(__CONFIG__.HTTPS_API_DOMAIN, {
      ...
      auth: (cb: (data: any) => void) => {
        ...
        console.log(
          `🟨 [SOCKET] auth() handshake fn invoked -> hasToken=${Boolean(
  ...
  static emit(event: string, data: any) {
    console.log(
      `🟦 [SOCKET][HTTP] emit OUT -> ${__CONFIG__.HTTPS_API_DOMAIN}/api/realtime${event}`,
      JSON.stringify(data),
    );
    ...
    yy.then((res: any) =>
      console.log(
        `🟩 [SOCKET][HTTP] emit RESPONSE <- ${event}`,
        JSON.stringify(res),
      ),
    ).catch((err: any) =>
      console.log(
        `🟥 [SOCKET][HTTP] emit ERROR <- ${event}`,
        err?.message ?? err,
      ),
    );
  }
  ...
  private static registerEvents(userID: string) {
    ...
    this.socket.onAny((eventName: string, ...args: any[]) => {
      console.log(
        `🟦 [SOCKET] EVENT IN <- userID=${userID} event="${eventName}"`,
        JSON.stringify(args),
      );
    });
