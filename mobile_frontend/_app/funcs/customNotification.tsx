...
const ToastCard = ({
  ...
  const pause = () => {
    ...
    remainingRef.current = Math.max(
      remainingRef.current - (Date.now() - lastResumeAtRef.current),
      0,
    );
    cancelAnimation(progress);
  };
...
export const Toastx = () => {
  ...
  useEffect(() => {
    pushToast = (opts: ToastOptions) => {
      ...
      const setter =
        opts.position === 'bottom' ? setBottomToasts : setTopToasts;
      setter(prev => [item, ...prev].slice(0, MAX_STACK));
    };
  }, []);

  const dismissTop = (id: number) =>
    setTopToasts(prev => prev.filter(t => t.id !== id));
  const dismissBottom = (id: number) =>
