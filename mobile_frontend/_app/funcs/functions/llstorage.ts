...
export class cacheStorage {
        ...
        if (profile === null || profile === false || profile === undefined) {
          ...
          if (!this.profileMemoryCache) {
            try {
              ...
              console.error(
                'Error reading fallback profile from AsyncStorage:',
                error,
              );
            }
          }
  ...
  public static getDeviceData = (forceRefresh = false): Promise<any> => {
    ...
    this.deviceLoadingPromise = (async () => {
      // storage cache
      if (!forceRefresh) {
        const getFromLocalStorage = await AsyncStorage.getItem(
          namer.storage.deviceSpecs,
        );
      ...
      try {
        ...
        if (!forceRefresh) {
          try {
            ...
            console.error(
              'Error reading getProducts from AsyncStorage:',
              error,
            );
          }
        }
        ...
        const response = await _http_request({
          customApiUrl:
            __CONFIG__.HTTPS_API_DOMAIN + '/api/core/v1/getProducts',
          reqType: 'POST',
        });
        ...
        if (response === null || response === false || response === undefined) {
          ...
          if (!this.productsMemoryCache) {
            try {
              ...
              console.error(
                'Error reading fallback products from AsyncStorage:',
                error,
              );
            }
          }
  ...
  public static getMapper = (
    ...
    this.mapperLoadingPromise = (async () => {
      try {
        ...
        if (!forceRefresh) {
          try {
            const cachedMapper = await AsyncStorage.getItem(
        ...
        const mapperData =
          response?.data?.mapper_payload ?? response?.mapper_payload;
        ...
        if (
          ...
          await AsyncStorage.setItem(
            namer.storage.mapper,
            JSON.stringify(this.mapperMemoryCache),
          );
        } else {
          this.mapperMemoryCache = this.mapperMemoryCache ?? null;
          if (!this.mapperMemoryCache)
            await AsyncStorage.removeItem(namer.storage.mapper);
    ...
    getMapper: async (forceRefresh = false): Promise<void> => {
      ...
      if (!forceRefresh) {
        try {
          ...
          console.error(
            'Error reading mapper_payload from AsyncStorage:',
            error,
          );
        }
      }
      ...
      if (server?.code === 200) {
        await AsyncStorage.setItem(
          namer.storage.mapper_payload,
          JSON.stringify(server?.mapper_payload),
        );
        ...
        xxa_logggingReport({
          type: 'function',
          extra: server,
          useraction: 'CONFIG.generateFromServer',
          url: __CONFIG__.HTTPS_API_DOMAIN + '/api/core/v1/getMapper',
          logMessage: 'Failed to fetch versioning data',
        });
      }
    },
  };
