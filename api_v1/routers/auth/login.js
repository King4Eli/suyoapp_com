...
login_router.post("/", async (req, res) => {
  ...
  const ipLimit = await checkRateLimit(
    `${namer.ratelimit.login_ip}${req.ip}`,
    envInt("LOGIN_IP_LIMIT", 30),
    envInt("LOGIN_IP_WINDOW_SECONDS", 600),
  );
  if (!ipLimit.allowed) {
  ...
  if (!vpincode || vpincode.length < 6) {
    const otpRequestLimit = await checkRateLimit(
      `${namer.ratelimit.login_otp_request}${phonenumber}`,
      envInt("LOGIN_OTP_REQUEST_LIMIT", 5),
      envInt("LOGIN_OTP_REQUEST_WINDOW_SECONDS", 600),
    );
    ...
    const [rows] = await db_pool.execute(
      "SELECT user_email, user_active FROM users WHERE user_phonenumber = ?",
      [phonenumber],
    );
    // @ts-ignore
    if (rows.length === 0) {
    ...
    const genPin = Math.floor(100000 + Math.random() * 999999)
      .toString()
      .slice(0, 6);
  ...
  const otpVerifyLimit = await checkRateLimit(
    `${namer.ratelimit.login_otp_verify}${phonenumber}`,
    envInt("LOGIN_OTP_VERIFY_LIMIT", 10),
    envInt("LOGIN_OTP_VERIFY_WINDOW_SECONDS", 600),
  );
