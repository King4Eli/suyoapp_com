...
export default async function pushFeedPost(data) {
    ...
    try {
        ...
        if (!limitCheck.allowed) {
            response.code = 429;
            response.message = "You've reached today's posting limit. Try again tomorrow.";
    ...
    `;
        ...
        if (result.affectedRows > 0) {
            ...
            response.postsRemainingToday = Math.max(0, FEED_POST_DAILY_LIMIT - limitCheck.count);
        }
    } catch (err) {
