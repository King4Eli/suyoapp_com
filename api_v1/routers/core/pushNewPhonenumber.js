  ...
  try {
    if (requestNewCode) {
      ...
      const [rows] = await db_pool.query(
        "SELECT user_id FROM users WHERE user_phonenumber = ? LIMIT 1",
        [newPhoneNumber],
      );
      ...
      const [result] = await db_pool.query(
        "UPDATE users SET user_phonenumber = ? WHERE user_id = ? AND user_phonenumber = ?",
        [newPhoneNumber, sessions.currentUserID, oldPhoneNumber],
      );
