...
export default async function pushProfile(input = {}) {
  ...
  try {
    ...
    if (profUpdates.length > 0) {
      ...
      // @ts-ignore
      const updates = [...latestByField.entries()].map(([field, value]) => ({
        field,
        value,
      }));
      ...
      const [result] = await db_pool.query(
        `UPDATE users SET ${setClauses} WHERE user_id = ?`,
        values,
      );
    ...
    if (hasKey(input, "prof_prompts") && Array.isArray(input.prof_prompts)) {
      ...
      await db_pool.query("DELETE FROM users_prompt WHERE user_id = ?", [
        sessions.currentUserID,
      ]);
      if (promptRows.length > 0) {
        await db_pool.query(
          "INSERT INTO users_prompt (user_id, prompts_variant_ref_id, answer) VALUES ?",
          [promptRows],
        );
      }
      savedSomething = true;
    ...
    if (
      ...
      await db_pool.query("DELETE FROM users_interests WHERE user_id = ?", [
        sessions.currentUserID,
      ]);
    ...
    if (hasKey(input, "prof_social_links")) {
      ...
      await db_pool.query(
        "UPDATE users SET user_bio_social_links = ? WHERE user_id = ?",
        [JSON.stringify(socialLinks), sessions.currentUserID],
      );
      savedSomething = true;
    }
