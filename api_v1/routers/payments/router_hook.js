  ...
  switch (event.type) {
    ...
    case "invoice.payment_succeeded": {
      ...
      try {
        ...
        tools.serverLog(
          `Error processing invoice.payment_succeeded: ${error}`,
          "hook_125",
        );
        return { success: false, error: "Processing error" };
      }
    ...
    case "invoice.payment_failed": {
      ...
      try {
        ...
        try {
          ...
          await connection.commit();
          tools.serverLog(
            `Subscription payment failed: ${subscriptionId}, status: ${newStatus}`,
            "hook_127",
          );
          return { success: true };
        } catch (dbError) {
        ...
        tools.serverLog(
          `Error processing invoice.payment_failed: ${error}`,
          "hook_129",
        );
        return { success: false, error: "Processing error" };
      }
