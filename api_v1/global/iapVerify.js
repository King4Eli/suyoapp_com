...
const APPLE_PRIVATE_KEY = (process.env.APPLE_APP_STORE_PRIVATE_KEY ?? '').replace(/\\n/g, '\n');
const APPLE_BUNDLE_ID = process.env.APPLE_BUNDLE_ID ?? 'com.vintolab.suyoapp';
const appleConfigured = Boolean(APPLE_KEY_ID && APPLE_ISSUER_ID && APPLE_PRIVATE_KEY);

function signAppleServerJwt() {
    const now = Math.floor(Date.now() / 1000);
    const header = { alg: 'ES256', kid: APPLE_KEY_ID, typ: 'JWT' };
    const payload = { iss: APPLE_ISSUER_ID, iat: now, exp: now + 300, aud: 'appstoreconnect-v1', bid: APPLE_BUNDLE_ID };
    const signingInput = `${base64UrlEncode(Buffer.from(JSON.stringify(header)))}.${base64UrlEncode(Buffer.from(JSON.stringify(payload)))}`;
    const signature = crypto.sign(null, Buffer.from(signingInput), { key: APPLE_PRIVATE_KEY, dsaEncoding: 'ieee-p1363' });
 ...
 */
export async function verifyAppleTransaction(transactionId, signedTransactionJws) {
    if (appleConfigured) {
        ...
        for (const host of ['https://api.storekit.itunes.apple.com', 'https://api.storekit-sandbox.itunes.apple.com']) {
            const resp = await fetch(`${host}/inApps/v1/transactions/${encodeURIComponent(transactionId)}`, {
                headers: { Authorization: `Bearer ${jwt}` }
            });
            if (resp.status === 404) continue; // not found here, try sandbox host
            if (!resp.ok) {
                tools.serverLog(`Apple App Store Server API error ${resp.status} for transaction ${transactionId}`, 'iap-apple-0');
        ...
        throw new Error('Transaction not found in production or sandbox App Store Server API');
    }

    if (IS_PRODUCTION) {
        throw new Error('Apple IAP verification is not configured (APPLE_APP_STORE_KEY_ID/ISSUER_ID/PRIVATE_KEY missing)');
    }

    tools.serverLog(`PSEUDO-MODE (unverified): trusting client-supplied Apple transaction ${transactionId}`, 'iap-apple-pseudo');
...
const GOOGLE_SERVICE_ACCOUNT_EMAIL = process.env.GOOGLE_PLAY_SERVICE_ACCOUNT_EMAIL ?? '';
const GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY = (process.env.GOOGLE_PLAY_SERVICE_ACCOUNT_PRIVATE_KEY ?? '').replace(/\\n/g, '\n');
const GOOGLE_PACKAGE_NAME = process.env.GOOGLE_PLAY_PACKAGE_NAME ?? 'com.vintolab.suyoapp';
const googleConfigured = Boolean(GOOGLE_SERVICE_ACCOUNT_EMAIL && GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY);
...
async function getGoogleAccessToken() {
    ...
    const signingInput = `${base64UrlEncode(Buffer.from(JSON.stringify(header)))}.${base64UrlEncode(Buffer.from(JSON.stringify(payload)))}`;
    const signature = crypto.sign('RSA-SHA256', Buffer.from(signingInput), GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY);
    ...
    if (!resp.ok) throw new Error(`Google OAuth token exchange failed with status ${resp.status}`);
    /** @type any */
    const body = await resp.json();
 ...
 */
export async function verifyGooglePurchase(productId, purchaseToken, isSubscription) {
    if (googleConfigured) {
        ...
        if (!resp.ok) {
            tools.serverLog(`Google Play Developer API error ${resp.status} for token ${purchaseToken}`, 'iap-google-0');
        ...
        if (isSubscription) {
            ...
            const expiresAtMs = lineItem?.expiryTime ? Date.parse(lineItem.expiryTime) : null;
            const isActive = body.subscriptionState === 'SUBSCRIPTION_STATE_ACTIVE' || body.subscriptionState === 'SUBSCRIPTION_STATE_IN_GRACE_PERIOD';
            return { verified: true, mode: 'verified', productId: lineItem?.productId ?? productId, isActive, expiresAtMs, raw: body };
        }
        // purchaseState: 0=purchased,1=canceled,2=pending
        return { verified: true, mode: 'verified', productId, isActive: body.purchaseState === 0, expiresAtMs: null, raw: body };
    }

    if (IS_PRODUCTION) {
        throw new Error('Google IAP verification is not configured (GOOGLE_PLAY_SERVICE_ACCOUNT_EMAIL/PRIVATE_KEY missing)');
    }

    tools.serverLog(`PSEUDO-MODE (unverified): trusting client-supplied Google purchase for ${productId}`, 'iap-google-pseudo');
    return { verified: false, mode: 'pseudo', productId, isActive: true, expiresAtMs: null, raw: { purchaseToken } };
}
