...
export class sessions {
    ...
    static set currentUserID(val) {
        ...
        if (!store) {
            console.error('🔴 sessions.currentUserID set outside of runInContext() -- value dropped to avoid cross-request leakage');
            return;
        }
        store.userId = val;
    }
    ...
    static createSession(userId) {
        if (!process.env.SESSION_ENCRYPT_HASH) {
            throw new Error('SESSION_ENCRYPT_HASH must be set -- refusing to issue an unsigned/forgeable session token.');
        }
        return jwt.sign(
            { user_id: userId },
    ...
    static verifyFullSession(auth_token = "") {
        if (!auth_token) {
            return { status: false, code: 400, message: "Authentication token is required." };
        }
        ...
        try {
            const decoded = jwt.verify(auth_token, process.env.SESSION_ENCRYPT_HASH, { algorithms: ['HS256'] });
            // @ts-ignore
