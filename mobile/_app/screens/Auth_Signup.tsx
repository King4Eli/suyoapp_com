...
// Option lists come from __MAPPER and are empty until it loads.
const EMPTY_OPTIONS: Record<string, MapperOption[]> = {
  intent: [],
  gender: [],
  interestedIn: [],
  smoking: [],
...
const mapperToOptions = (mapper: any, keys: string[]): MapperOption[] => {
  const mapperGroup = keys.map(key => mapper?.[key]).find(Boolean);
  if (!mapperGroup) return [];
  ...
  if (typeof mapperGroup === 'object') {
    return Object.entries(mapperGroup).map(([value, label]) => ({
      value: String(value),
      label: String(label),
    }));
  }
...
export const Auth_Signup = ({ route }: { route: any }) => {
  ...
  const [getMapper, setMapperOptions] = useState(EMPTY_OPTIONS);
  ...
  useEffect(() => {
    ...
    cacheStorage
      ...
      .then(mapper => {
        ...
        const genderOptions = mapperToOptions(mapper, ['bio_gender', 'gender']);

        setMapperOptions({
          intent: mapperToOptions(mapper, ['bio_intent', 'intent']),
          gender: genderOptions,
          // Same codes as bio_gender, plus -99 for everyone.
          interestedIn:
            genderOptions.length > 0
              ? [...genderOptions, { label: 'Everyone', value: '-99' }]
              : [],
          smoking: mapperToOptions(mapper, ['bio_smoking', 'smoking']),
          drinking: mapperToOptions(mapper, ['bio_drinking', 'drinking']),
          children: mapperToOptions(mapper, ['bio_children', 'children']),
          hasPet: mapperToOptions(mapper, ['bio_pets', 'pets']),
          // interests: mapperToOptions(mapper, ['interests', 'interest']),
        });
      })
      .catch(error => {
