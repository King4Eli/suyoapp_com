...
export function Screen_likes({ navigation }: { navigation: any }) {
  ...
  // Start bounce animation -- held in a ref so we can stop it on unmount /
  // navigation away, otherwise the loop keeps running in the background stack.
  useEffect(() => {
  ...
  const filteredLikes = useMemo(() => {
    ...
    if (activeFilter === 'verifiedOnly') {
      ...
      list = list
        ...
        .sort((a, b) => {
          const priorityDiff =
            getAllLikesSortPriority(b.item) - getAllLikesSortPriority(a.item);
  ...
  const handleEndReached = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      setVisibleLikes(prev => {
        ...
        return Math.min(
          prev + LIKES_PAGE_SIZE,
          filteredLikesCount || prev + LIKES_PAGE_SIZE,
        );
      });
    }, 1000);
      ...
      {getNewLikes.length > 0 ? (
        <>
          <FlatList
            ...
            keyExtractor={(item, index) =>
              item?.likedUserId || index.toString()
            }
                    ...
                    <View style={stylesoy.topChips}>
                      {isSuperlike(item) && (
                        <View
                          style={[
                            stylesoy.pill,
                            { backgroundColor: '#e11d48', opacity: 0.9 },
                          ]}
                        >
          ...
          {!activeSubscription && (
            ...
            >
              <Animated.View
                style={{ transform: [{ translateY: bounceInterpolatex }] }}
              >
                <Pressable
                  style={[
        ...
        <View style={stylesoy.emptyState}>
          <View style={stylesoy.emptyCard}>
            ...
            <Text style={stylesoy.emptySubtext}>
              Keep your profile active and check back soon for new likes.
            </Text>
            <Pressable
              style={stylesoy.emptyActionBtn}
