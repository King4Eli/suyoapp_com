...
function ConfettiPiece({
  ...
  useEffect(() => {
    ...
    opacity.value = withDelay(
      delay + fallDuration * 0.75,
      withTiming(0, { duration: fallDuration * 0.25 }),
    );
 ...
 */
export function ActionBurstOverlay({
  burst,
  onDone,
}: {
  burst: { kind: ActionBurstKind; key: number | string } | null;
  onDone: () => void;
}) {
  ...
  return (
    <View pointerEvents="none" style={styles.burstWrap}>
      <Animated.View style={mainStyle}>
        <View
          style={[styles.burstCircle, { backgroundColor: cfg.color + '26' }]}
        >
